...
const UPSTREAMS = {
  ...
  clankapp:      'https://api.clankapp.com',
};
...
const CACHE_TTL = {
  ...
  'api.clankapp.com':         60,  // big-chain whale feed — 1 minute (free tier 10/min)
};
...
export default {
  async fetch(request, env, ctx) {
    ...
    // ClankApp big-chain whale API: the key is an `api_key` query parameter, pulled
    // from the CLANKAPP_API_KEY secret so it never ships in the frontend bundle.
    if (target.host === 'api.clankapp.com') {
      const token = (env && env.CLANKAPP_API_KEY) || '';
      if (token) target.searchParams.set('api_key', token);
    }
