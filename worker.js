...
// Repair "mojibake": UTF-8 bytes that arrived decoded as Latin-1. Feeds are
// UTF-8, but the byte stream sometimes reaches us already mis-decoded, so smart
// punctuation (’ “ ” — …) and accents (é ñ) appear as high-Latin-1 sequences
// like â€™. Generic fix: re-pack each char as its low byte, then decode the
// byte array as UTF-8. This corrects ALL such cases without enumerating them.
// Uses only TextDecoder, which is available in the Workers runtime.
function fixMojibake(s) {
  ...
  try {
    ...
    for (let i = 0; i < s.length; i++) {
      ...
      if (c > 0xff) return s;   // already contains real multibyte glyphs — leave it
      bytes[i] = c;
    }
    const decoded = new TextDecoder('utf-8', { fatal: false }).decode(bytes);
    if (decoded.indexOf('\ufffd') !== -1) return s;  // decode failed — keep original
    return decoded;
  } catch (e) {
    return s;
  }
