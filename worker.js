...
function parseEtfTable(html) {
  // Grab the first <table> on the page (Farside's flow table is the main one).
  ...
  for (const row of rows) {
    ...
    // Header ticker row: first cell empty, contains known-looking codes. We detect
    // it as the row whose cells (after the first) are short alpha codes and which
    // is NOT the fee/date/summary row.
    if (!tickers && first === '' && cells.length > 3) {
      const candidate = cells.slice(1).map(c => c.trim());
      const looksLikeTickers = candidate.filter(c => /^[A-Z]{2,6}$/.test(c)).length >= 3;
      if (looksLikeTickers) {
        // drop trailing empty cells and a "Total" header if present
    ...
    if (ETF_DATE_RE.test(first)) {
      ...
      // Last numeric column is the Total.
  ...
  // Page lists oldest->newest; the latest REPORTED day is the last row whose total
  // is a real number (pending days show "-" => total null, but Farside prints 0.0).
  // We treat the last row with any non-null fund value as "latest reported".
  ...
  // Return latest + the trailing 30 days of history (most recent last).
  const recent = history.slice(Math.max(0, history.length - 30));
  return { latest, recent, tickers: tickers || [] };
}

async function handleEtf(which, origin, ctx) {
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (reqUrl.pathname === '/etf' || reqUrl.pathname.startsWith('/etf/')) {
      ...
      try {
        return await handleEtf(which, origin, ctx);
      } catch (e) {
