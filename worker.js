...
export default {
  async fetch(request, ctx) {
    ...
    // Stream the body without buffering the whole thing into memory.
    // For cacheable GETs, tee the stream: one branch to the client, one to cache.
    if (isGet && ttl > 0 && upstream.ok && cache && upstream.body) {
      try {
        respHeaders.set('Cache-Control', `public, max-age=${ttl}`);
        const [toClient, toCache] = upstream.body.tee();
        const cacheResp = new Response(toCache, { status: upstream.status, headers: respHeaders });
        ctx.waitUntil(cache.put(cacheKey, cacheResp));
        return new Response(toClient, { status: upstream.status, headers: respHeaders });
      } catch (e) {
        // tee/cache failed — fall through to plain passthrough below
      }
    }
